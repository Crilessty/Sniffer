...
void sniffer::tcp_viewer()
{
    ...
    while(1)
    {
        ...
        {
            tmp.sprintf("%c", (unsigned char)buffer[i]);
        ...
        for (i=0;i<4;i++)
        {
            tmp.sprintf("%c",(unsigned char)*((char *)(&ip->saddr)+i));
            ui->pack->insertPlainText(tmp);
        ...
        for (i=0;i<4;i++)
        {
            tmp.sprintf("%c",(unsigned char)*((char *)(&ip->daddr)+i));
            ui->pack->insertPlainText(tmp);
...
void sniffer::on_startsniffer_clicked()
{
    if (!state)
    {
